...
namespace DomainFramework.DataAccess
{
    ...
    /// (therefore we need DeleteCollection or DeleteCollectionAsync) and inserted after (Add or AddAsync)
    ...
    public interface ILinkedValueObjectCommandRepository : ICommandRepository
    {
        ...
        bool DeleteCollection(IAuthenticatedUser user, IUnitOfWork unitOfWork);

        Task AddAsync(IValueObject valueObject, IAuthenticatedUser user, IUnitOfWork unitOfWork);

        Task<bool> DeleteCollectionAsync(IAuthenticatedUser user, IUnitOfWork unitOfWork);
    }
}