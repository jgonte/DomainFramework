...
namespace BookWithPages.BookBoundedContext
{
    public class BookQueryRepository : EntityQueryRepository<Book, int?>
    {
        ...
        public override IEnumerable<Book> Get(QueryParameters queryParameters, IAuthenticatedUser user)
        {
            var result = Query<Book>
                .Collection()
                .Connection(BookWithPagesConnectionClass.GetConnectionName())
        ...
        public async override Task<IEnumerable<Book>> GetAsync(QueryParameters queryParameters, IAuthenticatedUser user)
        {
            var result = await Query<Book>
                .Collection()
                .Connection(BookWithPagesConnectionClass.GetConnectionName())
