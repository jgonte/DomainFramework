...
CREATE PROCEDURE [EmployeeBoundedContext].[pPerson_UnlinkSpouse]
    @personId INT
AS
BEGIN
    UPDATE [EmployeeWithSpouse].[EmployeeBoundedContext].[Person]
    SET
        [MarriedToPersonId] = NULL
    WHERE [MarriedToPersonId] = @personId;
...
CREATE PROCEDURE [EmployeeBoundedContext].[pPerson_GetSpouse]
    @personId INT
AS
BEGIN
    SELECT
    ...
    WHERE _q_.[MarriedToPersonId] = @personId;
