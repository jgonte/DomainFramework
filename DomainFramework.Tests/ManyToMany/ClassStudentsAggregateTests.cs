...
",
        ...
        {
            var context = new RepositoryContext("SqlServerTest.DomainFrameworkManyToManyTest.ConnectionString");

            context.RegisterCommandRepository<ClassEntity>(new ClassCommandRepository());

            context.RegisterCommandRepository<StudentEntity>(new StudentCommandRepository());

            context.RegisterCommandRepository<ClassEnrollmentEntity>(new ClassEnrollmentCommandRepository());
            ...
            var classQueryAggregate = new ClassEnrollmentQueryAggregate(context, classEntity);

            classQueryAggregate.Load(id);

            classEntity = classQueryAggregate.RootEntity;

            Assert.AreEqual(id, classEntity.Id);

            Assert.AreEqual("Programming", classEntity.Name);

            Assert.AreEqual(3, classQueryAggregate.Students.Count());

            var student = classQueryAggregate.Students.ElementAt(0);

            Assert.IsNotNull(student.Id);

            Assert.AreEqual("Sarah", student.FirstName);

            student = classQueryAggregate.Students.ElementAt(1);

            Assert.IsNotNull(student.Id);

            Assert.AreEqual("Yana", student.FirstName);

            student = classQueryAggregate.Students.ElementAt(2);
